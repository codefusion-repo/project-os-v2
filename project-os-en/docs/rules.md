...
The normal reading surface starts at the manifest and is limited to the selected resolution, applicable limits and evidence, output and artifact, exact template, requested skills, minimum live evidence, and target sources required by scope, validation, or source basis. Internal tooling, resolver-projected metadata, actual model context, and post-resolution reads are separate categories. Every execution retains them internally through `context_receipt.minimum_read_surface`, using repository-relative paths or live identifiers and reasons rather than absolute machine paths or full bodies. `context_plan` and the internal receipt remain intact at every level; `pm_facing_visibility` omits only the receipt's Markdown representation in `minimal` and `compact`, and shows it in full in the `full/debug` envelope. Reads outside the normal surface require `full/debug`, audit, debugging, security/authorization review, complex architecture, or a concrete PM decision; no reason replaces evidence or gates.
