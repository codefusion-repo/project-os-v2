...
- Optional: ADOPTION_ISSUE_NUMBER, PM_FEEDBACK_HUMANO, PM_QUESTION_HUMANO (PM feedback and questions are context only and never authorize an action)
...
**Repair:** When the audit finds the browser adapter missing or outdated, include a corrected draft in the adoption packet. When findings require a terminal correction and a live work unit exists (`ADOPTION_ISSUE_NUMBER`), draft a route prompt that delegates the repo-owned adapter repair to the terminal agent in `mode.delegated_commit_pr`, with branch preflight, validation, and exact PM approval; never invent the work unit.
