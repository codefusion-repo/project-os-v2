...
MOSDLC operation `draft-local-deploy-commands` · Phase 5 — Local, staging, and production deployment · Risk: medium.
...
**Does:** Draft the bundle of local deployment commands from the target-owned commands in Project-specific notes.
**For:** To prepare the execution of the local deployment without inventing commands.
**How:** Delegate the bundle to MOS-R.12 with `TARGET_ENVIRONMENT=local`
...
**Connections:** Previous: MOS-5.3. Next: MOS-5.11 or execution of the Human PM. Recommended: MOS-5.11 only with exact approval.
