...
**How:** Delegate review to MOS-R.11 with `TARGET_ENVIRONMENT=local`
and already reconstructed live relations; apply its evidence and gates.
Only pending human checks consume MOS-5.2; sufficient readiness
consumes MOS-5.10 in the same response without recapture. Every
...
**Connections:** Previous: QA/release or prior environment when required by the target. Next: MOS-5.2 only for pending human checks; MOS-5.10 with sufficient readiness. Recommended: the safe output resolved by MOS-R.11, without another selection.
