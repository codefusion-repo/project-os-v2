...
PM-facing traceability is `Reviewed evidence` and its per-output equivalent.
Reads stay limited to sources required by scope, validation, or source basis and
never expose absolute machine paths, bodies, secrets, or durable live state. When
an artifact requires an exact body—such as a route prompt—the template is its
only source.
