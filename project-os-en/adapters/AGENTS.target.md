...
The resolver accelerates resolution; the manifest remains canonical. Both provide shape only and never authorize an action. Follow resolved artifact, template, and skill references without copying their contracts here. A normal resolution does not return `context_plan`: detailed resolver provenance requires `--context-provenance <reason>` with a reason from `detailed_provenance_reasons`, and even then it is never proof of content delivered to the model.

After resolution, open only the applicable template, requested skills, and Project OS, target, or live-evidence sources required by scope, validation, or source basis. PM-facing traceability is `Reviewed evidence` and its per-output equivalent; do not add a receipt block at any level. When the PM asks for detailed provenance for audit, debugging, security or authorization review, or investigating an incorrect resolution, report the contract's `executor_reported_fields` alongside the requested `context_plan`, using repository-relative paths or live identifiers and short reasons rather than absolute machine paths or full bodies. Every additional read needs a reason allowed by the contract; do not recursively crawl Project OS by default.
