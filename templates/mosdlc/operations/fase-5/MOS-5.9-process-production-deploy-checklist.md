...
OPERATION:
  Classify Human PM production deployment checklist results into continue, correction, follow-up, no-op, or PM decision paths without mutating repository, GitHub, or environment state.
  Source map summary: Procesa el resultado de los pasos humanos del despliegue de produccion. Confirmar readiness o derivar gaps antes de continuar. Clasifica CHECKLIST_RESULT hacia continuar, corregir o detener.
...
LIVE_STATE:
  Read CHECKLIST_RESULT, production checklist source, target repository evidence, and PM decisions live.
  Treat the result as classification input only; this template does not verify production deployment by executing commands.
...
DO:
  ...
  For continue, recommend MOS-5.14 only when target-owned production deploy commands are expected to exist; production execution remains with the Human PM by default.
  ...
  Do not execute deployment, mutate GitHub, edit files, change host/DNS/provider/payment/auth settings, request secrets, expose values, run broad environment/config dumps, or invent production deploy commands.
...
LIMITS:
  Draft-only/read-only production checklist classification; no deployment behavior and no repository, GitHub, runtime-config, secret-store, host, DNS, provider, payment/auth, or external-service mutation.
  Template authority: none. Exact PM approval plus kernel gates are required before any separate write-capable operation.
  PM_FEEDBACK_HUMANO and PM_QUESTION_HUMANO are optional context only.

RECOMMENDED_NEXT_OPERATION:
  MOS-5.14 when target-owned command evidence exists; otherwise MOS-R.13 after Human PM production deployment or MOS-R.3 for a PM decision.
