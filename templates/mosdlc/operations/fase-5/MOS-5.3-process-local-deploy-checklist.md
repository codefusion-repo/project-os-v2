...
OPERATION:
  Classify Human PM local deployment checklist results into continue, correction, follow-up, no-op, or PM decision paths without mutating repository, GitHub, or environment state.
  Source map summary: Procesa el resultado de los pasos humanos del despliegue local. Confirmar readiness o derivar gaps antes de continuar. Clasifica CHECKLIST_RESULT hacia continuar, corregir o detener.
...
LIVE_STATE:
  Read CHECKLIST_RESULT, local checklist source, target repository evidence, and PM decisions live.
  Treat the result as classification input only; this template does not verify local deployment by executing commands.
...
DO:
  ...
  For continue, recommend MOS-5.10 only when target-owned local deploy commands are expected to exist; otherwise recommend Human PM execution or status.needs_context.
  ...
  Do not execute deployment, mutate GitHub, edit files, change config, request secrets, expose values, run broad environment/config dumps, or invent local deploy commands.
...
LIMITS:
  Draft-only/read-only local checklist classification; no deployment behavior and no repository, GitHub, runtime-config, secret-store, or external-service mutation.
  Template authority: none. Exact PM approval plus kernel gates are required before any separate write-capable operation.
  PM_FEEDBACK_HUMANO and PM_QUESTION_HUMANO are optional context only.

RECOMMENDED_NEXT_OPERATION:
  MOS-5.10 when target-owned command evidence exists; otherwise MOS-R.13 after Human PM local deployment or MOS-R.3 for a PM decision.
