...
def _mosdlc_template_paths() -> list[Path]:
    return sorted(MOSDLC_FASE6_DIR.glob("*.md"), key=lambda path: _mosdlc_sort_key(path.stem))
...
def test_mosdlc_fase6_migration_does_not_expand_kernel_or_unsupported_phases() -> None:
    ...
    migration_changed = any(
        path.startswith("templates/mosdlc/operations/fase-6/") for path in changed_files
    )
