"""MOSDLC.5 Fase 4 template migration guards."""
...
MOSDLC_FASE4_DIR = REPO_ROOT / "legacy-project-os" / "templates" / "mosdlc" / "operations" / "fase-4"
...
FASE4_IDS = [f"MOS-4.{index}" for index in range(1, 9)]
CHECKLIST_ROWS = {"MOS-4.1", "MOS-4.2", "MOS-4.3"}
PROCESSING_ROWS = {"MOS-4.4", "MOS-4.5", "MOS-4.6"}
FOLLOW_UP_ROW = "MOS-4.7"
CORRECTION_ROW = "MOS-4.8"
...
def _parse_operation_rows() -> dict[str, dict[str, str]]:
    ...
    for line in text.splitlines():
        if line.startswith("## "):
            in_fase4 = line.strip() == "## Fase 4 — Pruebas y QA"
            headers = None
            continue
        if not in_fase4:
            continue
...
def _template_path(row: dict[str, str]) -> Path:
    return MOSDLC_FASE4_DIR / f"{row['ID']}-{row['Operación']}.md"
...
def test_mosdlc_fase4_templates_are_documented_and_discoverable() -> None:
    ...
    for fragment in (
        "## Fase 4 Migrada",
        "templates/mosdlc/operations/fase-4/",
    ...
    for rid, row in rows.items():
        path = f"templates/mosdlc/operations/fase-4/{rid}-{row['Operación']}.md"
...
def test_fase4_map_rows_have_matching_mosdlc_templates() -> None:
    rows = _parse_operation_rows()
    assert list(rows) == FASE4_IDS
    assert [path.name for path in _mosdlc_template_paths()] == [
        f"{rows[rid]['ID']}-{rows[rid]['Operación']}.md" for rid in FASE4_IDS
    ]
...
def test_fase4_template_metadata_matches_source_map_kernel_contract() -> None:
    rows = _parse_operation_rows()
    for rid, row in rows.items():
...
def test_fase4_templates_use_required_fields_and_optional_human_context() -> None:
    rows = _parse_operation_rows()
    for rid, row in rows.items():
        text = _template_path(row).read_text(encoding="utf-8")
...
def test_fase4_templates_reference_only_valid_kernel_ids() -> None:
...
def test_fase4_templates_do_not_store_live_state_or_authority_grants() -> None:
    offenders: list[str] = []
...
def test_fase4_qa_boundaries_are_non_authorizing() -> None:
    rows = _parse_operation_rows()

    for rid in CHECKLIST_ROWS:
        text = _template_path(rows[rid]).read_text(encoding="utf-8")
        assert "no QA execution" in text
        assert "no claim that human QA was completed" in text
    ...
    for rid in PROCESSING_ROWS:
        ...
        assert "PM-executed" in text
        assert "Do not execute QA" in text

    follow_up = _template_path(rows[FOLLOW_UP_ROW]).read_text(encoding="utf-8")
    assert "output.pm_command_bundle" in follow_up
    assert "Human PM-executed" in follow_up
    assert "blocking QA failures" in follow_up
    assert "Do not execute the bundle" in follow_up

    correction = _template_path(rows[CORRECTION_ROW]).read_text(encoding="utf-8")
    assert "output.route_prompt" in correction
    assert "route prompts are non-authorizing outputs" in correction
    assert "PM_AUTHORIZATION_STATUS" in correction
    assert "Do not execute corrections" in correction


def test_fase4_production_readiness_does_not_migrate_deployment() -> None:
    rows = _parse_operation_rows()
    text = _template_path(rows["MOS-4.6"]).read_text(encoding="utf-8")
    for forbidden in (
        "workflow.deployment",
        "mode.delegated_deploy_execution",
        "evidence.deployment_readiness",
    ...
    assert "do not migrate or implement Fase 5 deployment behavior" in text
    assert "draft deploy commands" in text
    assert "execute deployment" in text
    assert "add deployment workflow ids" in text
    assert "MOS-R.4" in text


def test_fase4_templates_do_not_imply_kernel_growth() -> None:
    for path in _mosdlc_template_paths():
        text = path.read_text(encoding="utf-8")
...
def test_legacy_00_37_templates_remain_usable_and_fase4_sources_are_preserved() -> None:
    rows = _parse_operation_rows()
...
def test_mosdlc_fase4_migration_does_not_expand_kernel_or_unsupported_phases() -> None:
