...
POLICY_PATH = REPO_ROOT / "docs" / "VALIDATION_POLICY.md"
...
def _kernel_entry(filename: str, entry_id: str) -> dict:
    data = json.loads((REPO_ROOT / "kernel" / filename).read_text(encoding="utf-8"))
...
def test_kernel_and_agents_surface_the_global_policy() -> None:
    ...
    refs_by_actor = {
        ...
        for entry in json.loads((REPO_ROOT / "kernel" / "actors.json").read_text(encoding="utf-8"))["entries"]
    }
...
def test_route_and_review_prompts_classify_validation_without_blanket_defaults() -> None:
    route = _text("templates/route-prompt.md")
    op07 = _text("templates/operations/07-draft-issue-implementation-route-prompt.md")
    op09 = _text("templates/operations/09-review-pr-before-close-and-draft-package.md")
    mos34 = _text("templates/mosdlc/operations/fase-3/MOS-3.4-draft-implementation-route-prompt.md")
    mos37 = _text("templates/mosdlc/operations/fase-3/MOS-3.7-review-pr-before-close.md")
...
def test_mosdlc_docs_remove_new_phase_requires_tests_assumption() -> None:
    combined = "\n".join(
        _text(path)
        for path in (
            "docs/MOSDLC_TEMPLATE_STANDARD.md",
            "docs/PM_OPERATIONS.md",
            "docs/OPERATION_FLOWS.md",
            "docs/MOSDLC_OPERATION_MAP.md",
        )
    )
