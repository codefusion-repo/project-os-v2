...
- Una decisión posterior, explícita, exacta y aplicable de fuente viva verificable se vuelve `current_pm_decision`; conserva como `superseded_decision` únicamente la decisión anterior más reciente que también sea exacta y suficiente.
...
- Conserva `superseded_decision` y `current_pm_decision` cuando una decisión vigente ya se resolvió aunque un `remaining_gate` independiente produzca bloqueo. Usa `status.resolved` solo con decisión vigente exacta, supersesión inequívoca cuando aplique, salida soportada y demás gates satisfechos. Usa `status.needs_pm_decision` si la vigencia o clave material es ambigua; `status.needs_context` si falta fuente, procedencia, orden o relación verificable; y `status.blocked` para mutación con aprobación no verificable o exacta insuficiente, actor/mode/superficie/permisos incompatibles, secretos, validación fallida, límite no delegable o extensión a otra acción.
