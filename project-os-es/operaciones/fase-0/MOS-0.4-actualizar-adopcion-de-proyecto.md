...
**Drift terminal y route prompt:** Reporta `terminal_adoption_state` y rutea únicamente el drift terminal confirmado. Cuando exista una unidad de trabajo viva (`ADOPTION_ISSUE_NUMBER`), draftea un route prompt que delega la actualización de adapters repo-owned al terminal agent en `mode.delegated_commit_pr`, con branch preflight, validación y aprobación PM exacta; conserva las notas y constraints target-owned y nunca inventes la unidad de trabajo.
