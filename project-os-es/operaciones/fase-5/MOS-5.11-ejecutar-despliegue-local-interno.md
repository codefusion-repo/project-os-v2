# MOS-5.11 — Ejecutar el despliegue local (interno)

Operación MOSDLC `execute-local-deploy` · Fase 5 — Despliegue local / staging / producción · Riesgo: medium.
...
**Hace:** Ejecuta el despliegue local/debug por terminal agent solo si el target lo soporta.
**Para:** Probar despliegues locales de dogfood interno sin pasos manuales repetitivos.
**Cómo:** Reconstruye la misma unidad y consume la evidencia de MOS-R.11/
MOS-R.12 para `local` según la continuidad del contrato común.
...
**Cuida**
- Ejecuta solo los comandos target-owned aprobados para el entorno local, un entorno a la vez; nunca inventes, adivines ni amplíes comandos de despliegue.
...
**Conexiones:** Antes: MOS-5.10. Después: MOS-R.13. Recomendada: MOS-R.13.
