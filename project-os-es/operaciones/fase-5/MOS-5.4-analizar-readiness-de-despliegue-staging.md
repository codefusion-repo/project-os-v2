# MOS-5.4 — Analizar readiness de despliegue staging

Operación MOSDLC `analyze-staging-deploy-readiness` · Fase 5 — Despliegue local / staging / producción · Riesgo: medium.
...
**Hace:** Analiza readiness y configuración necesaria del despliegue staging.
**Para:** Preparar un despliegue staging seguro y reproducible.
**Cómo:** Delega la revisión en MOS-R.11 con `TARGET_ENVIRONMENT=staging`
y las relaciones vivas ya reconstruidas; aplica su evidencia y gates.
Solo comprobaciones humanas pendientes consumen MOS-5.5; readiness
suficiente consume MOS-5.12 en la misma respuesta, sin recaptura.
...
**Conexiones:** Antes: QA/release o entorno previo si el target lo exige. Después: MOS-5.5 solo para checks humanos pendientes; MOS-5.12 con readiness suficiente. Recomendada: la salida segura resuelta por MOS-R.11, sin otra selección.
