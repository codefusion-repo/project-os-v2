...
a los mismos templates y este archivo no cambia la numeracion `00`-`35`.
...
| Handoff/evaluation | 17 transfiere contexto; 05 revisa estado. | Nuevo Operation 35 recomienda la siguiente operacion solamente; nunca ejecuta transiciones. |

No sobreviven como operaciones en este corte: `status.needs_pm_decision`
generico, porque la decision debe volver a la operacion que produjo el status; y
phase readiness review, porque los gates existentes ya fallan cerrado y falta
dogfood que pruebe friccion repetida. Ambos quedan como reglas de docs/tests o
seguimiento diferido, no como ids de kernel.
...
| 34 | Manual implementation planning | Humano PM entrega evidencia de implementacion manual aplicada despues de 33. | `templates/operations/34-process-manual-implementation-result.md` | `evidence.issue_scope`, `evidence.source_basis`, `evidence.repo_state` | Req: `ISSUE_NUMBER`, `MANUAL_IMPLEMENTATION_RESULT`; Opt: `MANUAL_IMPLEMENTATION_PLAN`, `PR_NUMBER`, `TARGET_REPOSITORY`, `PM_FEEDBACK_HUMANO`, `PM_QUESTION_HUMANO` | `output.status_result`, `output.route_prompt`, `output.pm_command_bundle` | 09 si existe PR; 08 para correcciones; 21 para follow-up; gates QA/security/design cuando falte esa evidencia. | Si issue, resultado manual, plan, repo, PR o validacion no pueden inspeccionarse suficientemente, devolver `status.needs_context`; riesgo de secretos devuelve `status.blocked`. | Draft-only; no afirma que browser chat aplico o valido cambios y no reemplaza 09 cuando existe PR. |
...
|---|---|---|---|
| Process terminal-agent execution report outside PR review | Docs/template clarification plus test guard; no new operation for the normal PR path. | When a PR exists, Operation 09 is already the review-before-close path and consumes execution reports as evidence leads. No concrete non-PR/pre-review use case was proven in this scope. | Operation 09 and docs state the standard path; tests guard against duplicate execution-report processing. |
...
| Process `status.needs_pm_decision` | Docs/test clarification; no named operation. | PM decisions belong to the operation that produced the status. A generic status processor would cross workflow families and risk hidden workflow-engine behavior. | SDLC fit docs and tests require this to stay inside originating operations. |
...
| Phase readiness review | Deferred follow-up. | Existing operations already fail closed on missing evidence, and no repeated dogfood friction proves a separate readiness gate yet. | Docs/tests state that any future gate must remain advisory/read-only and fail closed. |
...
classification by operation `34`, and recommendation-only lifecycle selection by
operation `35`. None of these operations executes lifecycle transitions
automatically.

## Deferred Lifecycle Follow-up Candidates

| Deferred candidate | Why deferred | Required evidence before adding |
|---|---|---|
| Phase readiness review | Current gate operations already fail closed and can report missing evidence. | Repeated dogfood examples showing PM friction before implementation, QA, security, closeout, or release that existing operations cannot express. |
| Terminal execution report outside PR review | Normal PR execution reports belong to Operation 09. | A concrete non-PR or pre-review report-processing case where 09, 08, 21, and 35 cannot safely route the work. |
