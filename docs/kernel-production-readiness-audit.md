...
| `boundary.validation_discipline` | Requires global proportional validation and honest PM-run/manual validation reporting. | All agents, target repositories, terminal agents and reviewers. | Changed behavior and scoped validation evidence. | Validation gate. | `docs/VALIDATION_POLICY.md`, execution report and review outputs. | Kernel-level with doc detail. | keep |
...
| Test surface | Tests protect important invariants but many assertions depend on exact prose fragments. | Split semantic invariants from phrase-level regression checks; add/update tests only for deterministic or safety-relevant contracts per `docs/VALIDATION_POLICY.md`. | split | yes |
